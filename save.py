...
class GPLayer(gpytorch.models.GridInducingVariationalGP):
...
class DKLModel(gpytorch.Module):
        def __init__(self):
            ...
            n_features = 5
            self.feature_extractor = Deepgs(n_features,dropout=False)
            self.gp_layer = GPLayer(n_features)

        def forward(self, x):
            features = F.relu(self.feature_extractor(x.float()))
